...
- (instancetype) initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.pageControl=[[UIPageControl alloc]initWithFrame:CGRectMake((kwidth - 60)/2, kheight - 28, 60, 20)];
        self.pageControl.currentPageIndicatorTintColor=[UIColor colorWithWhite:0 alpha:0.7];
        self.pageControl.pageIndicatorTintColor=[UIColor whiteColor];
        
        self.currentIndex = 0;
        
        [self.pageControl addTarget:self action:@selector(valueChanged:) forControlEvents:(UIControlEventAllTouchEvents)];
...
- (void)valueChanged:(UIPageControl *)page{
    NSLog(@"ppppppp%ld",(long)page.currentPage);
...
-(void)setImageByIndex:(int)currentIndex array:(NSArray *)array
{
   ...
   // self.centerImageView.image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:currentImageName]]];
    
    [self.centerImageView setImageWithURL:[NSURL URLWithString:currentImageName]];
...
- (UIImageView *)centerImageView {
    if (!_centerImageView) {
        _centerImageView = [[UIImageView alloc]initWithFrame:CGRectMake(kwidth+8, 8, kwidth-16, kheight-16)];
       // [_centerImageView.layer setMasksToBounds:YES];
        ...
        _centerImageView.layer.shadowOffset=CGSizeMake(0, 3);
        _centerImageView.layer.shadowOpacity=0.8;
        _centerImageView.layer.shadowRadius=4.f;
        
         [_centerImageView.layer setMasksToBounds:NO];

        _centerImageView.layer.cornerRadius=6.f;
...
- (UIImageView *)LeftImageView {
    if (!_LeftImageView) {
        _LeftImageView = [[UIImageView alloc] initWithFrame:CGRectMake(8, 8, kwidth-16, kheight-16)];
       // [_LeftImageView.layer setMasksToBounds:YES];
        ...
        _LeftImageView.layer.shadowOffset=CGSizeMake(0, 2);
        _LeftImageView.layer.shadowOpacity=0.8;
        _LeftImageView.layer.shadowRadius=3.f;
        _LeftImageView.layer.cornerRadius=4.f;

        [_LeftImageView.layer setMasksToBounds:NO];
        
        _LeftImageView.layer.cornerRadius=6.f;
    }
...
- (UIImageView *)rightImageView {
    if (!_rightImageView) {
        _rightImageView = [[UIImageView alloc] initWithFrame:CGRectMake(2 * kwidth+8, 8, kwidth-16, kheight-16)];
        ...
        _rightImageView.layer.shadowColor=[UIColor grayColor].CGColor;
        _rightImageView.layer.shadowOffset=CGSizeMake(0, 2);
        _rightImageView.layer.shadowOpacity=0.8;
        _rightImageView.layer.shadowRadius=3.f;
        _rightImageView.layer.cornerRadius=4.f;

        [_rightImageView.layer setMasksToBounds:NO];
        
        _rightImageView.layer.cornerRadius=6.f;
    }
